...
class BaseImporter(object):
    ...
    def __init__(self, src_path, repository_path, gpg_homedir, keyring, sign_key):
    ...
    def _apply_commit_to_repo(self, commit, ref):
        ...
        # Skip the rest of processing if the current ref is the same as in delta
        ...
        if current_rev == commit:
            logging.info('Ref %s already at commit %s. Skipping changes.', ref, commit)
            return
        ...
        try:
            ...
            # Importer-specific way to get data from provided commit in source repo
            # into the target_repo
            self._import_commit(commit, src_path_obj, target_repo)

            # Verify that the commit signature is valid
            verify_commit_sig(target_repo, commit, self._gpg_homedir, self._keyring)
            ...
            if self._sign_key:
                logging.info("Signing with key %s from %s", self._sign_key, self._gpg_homedir)
                try:
